...
export default function ProfileViewPage() {
  ...
  const handleCopyLink = () => {
    if (!user) return;
    const shareUrl = `${window.location.origin}/profile/share/${user._id}`;
    navigator.clipboard.writeText(shareUrl);
    addToast({
      title: "Link Copied",
      description: "Profile sharing link copied to clipboard!",
      color: "success",
    });
  };
    ...
    <div className="min-h-screen bg-[whitesmoke]">
      ...
      <nav className="bg-white border-b border-gray-100 px-8 py-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-lg font-bold text-slate-900 tracking-tight">Chasescroll Details</span>
        </div>
        <div className="flex items-center gap-4">
          <Button variant="light" isIconOnly radius="full" onPress={handleCopyLink}>
              ...
              <CardBody className="p-10">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="p-6 bg-slate-50/50 rounded-2xl border border-slate-100 flex items-center gap-4">
                    ...
                    <div>
                      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">Role</p>
                      <p className="text-sm font-bold text-slate-900">{user?.position.toUpperCase()}</p>
                    </div>
                  </div>
                  {/* <div className="p-6 bg-slate-50/50 rounded-2xl border border-slate-100 flex items-center gap-4">
                    <div className="bg-indigo-100 p-2.5 rounded-xl text-indigo-600">
